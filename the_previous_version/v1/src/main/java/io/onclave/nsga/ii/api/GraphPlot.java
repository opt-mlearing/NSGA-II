...
public class GraphPlot extends ApplicationFrame {
    ...
    private void createDataset(final Population population, String key, XYSeriesCollection dataset) {
        ...
        population.getPopulace().stream().forEach((c) -> {
            paretoFront.add(Configuration.getObjectives().get(0).objectiveFunction(c), Configuration.getObjectives().get(1).objectiveFunction(c));
        });
