...
public class Reporter {
    ...
    public static void reportSingularFront(List<Chromosome> singularFront, int i) {
        ...
        for (Chromosome c : singularFront) {
            ...
            for (Chromosome dc : c.getDominatedChromosomes())
                p("\t\tslave : " + ++j + " | " + dc.getUniqueID() + " | " + Integer.toString(dc.getDominationRank()));
        }
    }
        ...
        for (int i = 1; i <= max; i++) {
            ...
            if (population != null && !population.isEmpty()) {
                ...
                for (Chromosome c : population) {
                    ...
                    for (Chromosome dc : c.getDominatedChromosomes())
                        p("\t\tslave : " + ++d + " | " + dc.getUniqueID() + " | " + Integer.toString(dc.getDominationRank()));
                }
            }
        }
    }
