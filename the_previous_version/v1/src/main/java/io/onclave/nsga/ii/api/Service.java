...
public class Service {
            ...
            for (Chromosome competitor : populace)
                if (!competitor.equals(chromosome)) {
                    ...
                    if (dominates(chromosome, competitor)) {
                        if (!chromosome.getDominatedChromosomes().contains(competitor))
                            chromosome.getDominatedChromosomes().add(competitor);
                    } else if (dominates(competitor, chromosome))
                        chromosome.setDominationRank(chromosome.getDominationRank() + 1);
                }
        ...
        while (previousFront != null && !previousFront.isEmpty()) {
            ...
            for (Chromosome chromosome : previousFront) {
                ...
                for (Chromosome recessive : chromosome.getDominatedChromosomes()) {
                     ...
                     */
                    if (recessive.getDominationRank() != 0)
                        recessive.setDominationRank(recessive.getDominationRank() - 1);
                    if (recessive.getDominationRank() == 0)
                        if (!nextFront.contains(recessive)) nextFront.add(recessive);
                }
            }
            ...
            if (nextFront.isEmpty() && !isDominatedChromosomesEmpty(previousFront)) {
                ...
                for (Chromosome chromosome : previousFront)
                    while (hasRecessiveRankGreaterThanZero(chromosome))
                        for (Chromosome recessive : chromosome.getDominatedChromosomes())
                            if ((minimumRank == -1) || minimumRank > recessive.getDominationRank())
                                minimumRank = recessive.getDominationRank();

                if (minimumRank != -1) for (Chromosome chromosome : previousFront)
                    while (hasRecessiveRankGreaterThanZero(chromosome))
                        for (Chromosome recessive : chromosome.getDominatedChromosomes()) {
                            if (recessive.getDominationRank() != 0)
                                recessive.setDominationRank(recessive.getDominationRank() - minimumRank);
                            if (recessive.getDominationRank() == 0)
                                if (!nextFront.contains(recessive)) nextFront.add(recessive);
                        }
            }
             ...
             */
            if (!nextFront.isEmpty()) paretoFront.put(++i, nextFront);
            else break;
    ...
    public static List<ParetoObject> crowdingDistanceAssignment(List<Chromosome> singularFront) {
        ...
        for (IObjectiveFunction objective : objectives) {
            ...
            for (ParetoObject paretoObject : singlePareto) {
                ...
                if ((maxObjectiveValue == null) || (maxObjectiveValue < objective.objectiveFunction(paretoObject)))
                    maxObjectiveValue = objective.objectiveFunction(paretoObject);
                if ((minObjectiveValue == null) || (minObjectiveValue > objective.objectiveFunction(paretoObject)))
                    minObjectiveValue = objective.objectiveFunction(paretoObject);
            }
    ...
    public static List<ParetoObject> crowdComparisonSort(List<ParetoObject> singleFront) {
         ...
         */
        singleFront.stream().forEach((paretoObject) -> {
            paretoObject.setCrowdingDistanceSorted(false);
        });
            ...
            if (!presentParetoObject.isCrowdingDistanceSorted()) {
                ...
                for (int j = 0; j < singleFront.size(); j++) {
                    ...
                    if (!competitor.isCrowdingDistanceSorted()) {
                        ...
                        if (i != j)
                            if ((dominationRank > competingDominationRank) || ((dominationRank == competingDominationRank) && (crowdingDistance < competingCrowdingDistance)))
    ...
    public static double decodeGeneticCode(final Allele[] geneticCode) {
        ...
        for (int i = 0; i < binaryString.length(); i++)
            if (binaryString.charAt(i) == '1') value += Math.pow(2, binaryString.length() - 1 - i);
